...
function injectedApplyTemplate(config) {
  ...
  async function chooseDropdownOption(host, targetValue) {
    if (!host || !clickDropdownTrigger(host)) {
      return false;
    }
    ...
    const option =
      ...
      Array.from(shadow?.querySelectorAll("kat-option") || []).find(
  ...
  function chooseResolvedStatus() {
    const radio =
      document.querySelector(`input[name="caseStatusRadioGroup"][value="${CASE_STATUS_VALUE}"]`) ||
  ...
  (async () => {
    ...
    const dropdowns = Array.from(document.querySelectorAll("kat-dropdown"));
    const categoryDropdown =
      dropdowns.find((node) => node.getAttribute("label") === "Reason category") ||
      dropdowns[3];

    await chooseDropdownOption(categoryDropdown, CATEGORY_VALUE);
    await sleep(1000);

    const refreshedDropdowns = Array.from(document.querySelectorAll("kat-dropdown"));
    const reasonDropdown =
      refreshedDropdowns.find((node) => node.getAttribute("label") === "Reason code") ||
      refreshedDropdowns[4];

    if (reasonDropdown) {
      const reasonOptions = JSON.parse(reasonDropdown.getAttribute("options") || "[]");
      const matchedReason = reasonOptions.find((item) => item.name === config.reasonText);

      if (matchedReason) {
        await chooseDropdownOption(reasonDropdown, String(matchedReason.value));
      }
    }
