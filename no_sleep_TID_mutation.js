(async () => {
  ...
  // ✅ MutationObserver 等容器“静默”后再读取（更快更准）
  async function waitContainerIdle(container, idleMs = 450, timeoutMs = 9000) {
    return new Promise((resolve) => {
      ...
      try {
        ...
        // 如果 observe 失败就直接 fallback
        resolve(true);
        return;
      }

      // 如果开始时就没变化，也让它在 idleMs 后结束
      timer = setTimeout(finish, idleMs);
  ...
  for (const t of targets) {
    ...
    // ✅ 等容器静默再读取（更快更准）
    await waitContainerIdle(container, 450, 9000);
